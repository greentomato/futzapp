 ...
 * %1$s: fecha y d韆 en que se juega el partido [lunes - viernes] [1 - 31]
 * %2$s: hora [HH:mm a]
...
var meBajoMessage = "Me bajo del partido del d韆 %1$s de %2$s a las %3$s en %4$s<br/>Entra a Futzapp y mira como quedaron los equipos: %5$s";
...
var juegoMessage = "Juego el partido del d韆 %1$s de %2$s a las %3$s en %4$s<br/>Entra a Futzapp y mira como quedaron los equipos: %5$s";
...
var completoMessage = "Ya estamos todos para el partido del d韆 %1$s de %2$s a las %3$s en %4$s<br/>Entra a Futzapp y mira como quedaron los equipos: %5$s";
(function() {
	'use strict';
	...
	function($rootScope, $scope, $location, Users, Matches, Teams, Notifications) {
	    ...
	    $scope.answerNo = function(match){
		    ...
		    Matches.updateGuests(data, function(response){
				...
				/* si estaba anotado, y se baj贸 => envio mail al admin */
				if(wasConfirmed)
    ...
    function($rootScope, $scope, $routeParams,  $filter, $sanitize, $location, Matches, Teams, Users, Notifications) {
	    ...
	    $scope.addPlayer = function(playerName){
	    	if(playerName == "" || playerName == undefined){
	    		showAlert("Error", "Ten茅s que poner el nombre de tu amigo!");
	    	} else {
	    		...
	    		Users.save(user, function(response){
	    			...
	    			Matches.updateGuests(data, function(teamGuests){
	    				//agregar usuario al equipo elegido o al que le falten m谩s jugadores
...
fulboServices.factory('Notifications', ['$rootScope', '$filter', '$sanitize', '$location', 'MandrillHelper', function($rootScope, $filter, $sanitize, $location, MandrillHelper) {
	var sdo = {
		...
		cancelado: function(match, guests){
			var message = "Se cancela el partido del d铆a " + $filter('dateFormat')(match.date, 'dddd d') + " de " + $filter('dateFormat')(match.date, 'MMMM') +  " a las " + $filter('dateFormat')(match.date, 'hh:mm a') + " en " + match.field.name;
